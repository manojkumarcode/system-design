...
```mermaid
mindmap
  root((Cloud-Native<br/>Deployment))
    (1. Rolling Update)
    ...
    (2. Blue-Green)
      ...
      Rollback in SECONDS
      Costs ~2x during overlap
      Azure slots / CodeDeploy
    (3. Canary)
      1 to 5 percent traffic first
      Compare against baseline
      Automated metric analysis
      Argo Rollouts / Flagger
      Smallest blast radius
    (4. Feature Flags)
      Different axis - code path
      Deploy is not release
      Fastest rollback of all
      Composes with 1 2 3
      Watch for flag debt
    (5. GitOps)
      Different axis - delivery
      Git is source of truth
      Pull model, no cluster creds
      Rollback = git revert
      Argo CD / Flux
    (Cross-cutting)
      Expand and contract DB
      Version-label all telemetry
      Immutable digest-pinned images
      Business metrics not just 5xx
```
